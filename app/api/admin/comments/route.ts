...
export async function POST(request: NextRequest) {
  ...
  if (svc) {
    ...
    if ((count ?? 0) >= 5) {
      return NextResponse.json(
        { error: "You're commenting too quickly. Please wait a few minutes and try again." },
        { status: 429 }
      );
    }
  }
  ...
  if (urlMatches.length > 2) {
    return NextResponse.json({ error: "Comments can't contain more than 2 links." }, { status: 400 });
  }
  if (/(.)\1{9,}/.test(text)) {
    return NextResponse.json({ error: "That comment looks like spam. Please rewrite it." }, { status: 400 });
  }
